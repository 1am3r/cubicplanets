 ...
 */
	...
	Real VertexIndexToShape::getRadius()
	{
		...
		{
			getSize();
			mBoundRadius = Ogre::Real(std::max(mBounds.x,std::max(mBounds.y,mBounds.z)) * 0.5);
		}
		return mBoundRadius;
	}
	//------------------------------------------------------------------------------------------------
						 ...
						 Vector3 &box_kCenter)
	{
		...
		if (!getBoneVertices(bone, vertex_count, vertices, bonePosition))
...

		 {
			 ...
			 const Ogre::Real invVertexCount = Ogre::Real(1.0 / vertex_count);
		...
		box_kCenter += (Ogre::Real(0.5)*(fY0Max+fY0Min))*box_akAxis[0] +
			(Ogre::Real(0.5)*(fY1Max+fY1Min))*box_akAxis[1] +
			(Ogre::Real(0.5)*(fY2Max+fY2Min))*box_akAxis[2];
