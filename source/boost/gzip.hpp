...
public:
    ...
    std::streamsize write(Sink& snk, const char_type* s, std::streamsize n)
    {
        if (!(flags_ & f_header_done)) {
            ...
            offset_ += static_cast<size_t>(boost::iostreams::write(snk, header_.data() + offset_, amt));
        ...
        std::streamsize read(char* s, std::streamsize n)
        {
            ...
            if (offset_ < pbsize) {
                ...
                BOOST_IOSTREAMS_CHAR_TRAITS(char)::copy(
                    s, putback_.data() + offset_, static_cast<size_t>(result));
                offset_ += static_cast<size_t>(result);
                if (result == n)
                    return result;
            }
